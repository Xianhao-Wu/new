...
from model import model as myNet
import losses
...
parser.add_argument('--train_dir', default='/root/autodl-tmp/dataset/train', type=str, help='Directory of train images')
parser.add_argument('--val_dir', default='/root/autodl-tmp/dataset/test', type=str, help='Directory of validation images')
...
for epoch in range(start_epoch, num_epochs + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        loss_fft = criterion_fft(restored[0], target[0]) + criterion_fft(restored[1], target[0]) + criterion_fft(restored[2], target[0])+criterion_fft(restored[3], target[1]) + criterion_fft(restored[4], target[2])
        loss_char = criterion_char(restored[0], target[0]) + criterion_char(restored[1], target[0]) + criterion_char(restored[2], target[0])+criterion_char(restored[3], target[1]) + criterion_char(restored[4], target[2])
        loss_edge = criterion_edge(restored[0], target[0]) + criterion_edge(restored[1], target[0]) + criterion_edge(restored[2], target[0])+criterion_edge(restored[3], target[1]) + criterion_edge(restored[4], target[2])
